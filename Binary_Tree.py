...
def Breadth_First_Search(Node):
    ...
    return path
print("Breadth First Search:"+str(Breadth_First_Search(a)))
...
print("Deep First Search:"+str(Deep_First_Search(a)))


# Find if a node is in the Tree

def has_node_recurssive(Node,target):
    ...
    if Node is None:
        ...
        return (has_node_recurssive(Node.left,target)) or (has_node_recurssive(Node.right,target))
j = Node("j")
for node in [f,j]:
    if has_node_recurssive(a,node):
        print("Node "+ str(node.value).upper()+" is in the Binary Tree")
    else:
        print("Node "+str(node.value).upper()+ " is not in the Binary Tree")

def has_node_iterator(Node,target):
    sequence = deque()
    if Node is None:
        return False
    ...
    while len(sequence)>0:
        ...
        if not current.left is None:
            sequence.append(current.left)
        if not current.right is None:
            sequence.append(current.right)
    return False
for node in [f,j]:
    if has_node_iterator(a,node):
        print("Node "+ str(node.value).upper()+" is in the Binary Tree")
    else:
        print("Node "+str(node.value).upper()+ " is not in the Binary Tree")

# Calculate the sum of the tree
# Define a new tree with node key and value
print("   3")
print("  /\\")
print(" 11 4")
print(" /\\  \\")
...
class Weighted_Node():
    def __init__(self,key,value):
        ...
        self.left = None
        self.right=None
Weight_Node1 = Weighted_Node("a",3)
Weight_Node2 = Weighted_Node("b",11)
Weight_Node3 = Weighted_Node("c",4)
Weight_Node4 = Weighted_Node("d",4)
Weight_Node5 = Weighted_Node("e",2)
Weight_Node6 = Weighted_Node("f",1)
Weight_Node1.left = Weight_Node2
Weight_Node1.right = Weight_Node3
Weight_Node2.left = Weight_Node4
Weight_Node2.right = Weight_Node5
Weight_Node3.right = Weight_Node6

def tree_sum_recursive(Node):
    if Node is None:
        return 0
    else:
        return tree_sum_recursive(Node.left)+Node.value+tree_sum_recursive(Node.right)
print("The Sum of the Tree is "+ str(tree_sum_recursive(Weight_Node1)))

def tree_sum_iteraotr(Node):
    sum = 0
    if Node is None:
        return 0
    sequence = deque()
    ...
    while len(sequence)>0:
        ...
        if not current.right is None:
            sequence.append(current.right)
    return sum
print("The Sum of the Tree is "+ str(tree_sum_iteraotr(Weight_Node1)))

# Tree Minimum and Maximum value

def Min_Max_Node_Recurssive(Node):
    if Node is None:
        return math.inf, -math.inf
    mini = min(Min_Max_Node_Recurssive(Node.left)[0],Node.value,Min_Max_Node_Recurssive(Node.right)[0])
    maxi=  max(Min_Max_Node_Recurssive(Node.left)[1],Node.value,Min_Max_Node_Recurssive(Node.right)[1])
    return mini,maxi
print("The Minimum Node is {}, and The Maximum Node is {}".format(Min_Max_Node_Recurssive(Weight_Node1)[0],Min_Max_Node_Recurssive(Weight_Node1)[1]))

def Min_Max_Node_Iterator(Node):
    minimum = math.inf
    maximum = -math.inf
    if Node is None:
        return min, max
...
print("The Minimum Node is {}, and The Maximum Node is {}".format(Min_Max_Node_Iterator(Weight_Node1)[0],Min_Max_Node_Iterator(Weight_Node1)[1]))

# Find the max root to leaf

def mini_max_root_to_leaf(Node):
    if Node is None:
        return math.inf,-math.inf
    if Node.left is None and Node.right is None:
        return Node.value,Node.value
    min_child = min(mini_max_root_to_leaf(Node.left)[0],mini_max_root_to_leaf(Node.right)[0])
    max_child = max(mini_max_root_to_leaf(Node.left)[1], mini_max_root_to_leaf(Node.right)[1])
    return min_child+Node.value,max_child+Node.value

print("The Minimium path is {} and The Maximum path is {}".format(mini_max_root_to_leaf(Weight_Node1)[0],mini_max_root_to_leaf(Weight_Node1)[1]))